...
    // === CHECK ROW ===
    let left = Math.floor(index / sizex) * sizex;
    let right = Math.floor(index / sizex + 1) * sizex - 1;
...
    let top = index - (index % sizex);
    let bottom = index + sizex * (sizey - 1 - Math.floor(index / sizex));
...
    // === CHECK DIAGONAL \ ===
    cnt = 0; p3 = undefined; blocked = false; half_block = false; max_cnt = 0;
    for (let i = index - distance * (sizex + 1); i <= index + distance * (sizex + 1); i += sizex + 1) {
